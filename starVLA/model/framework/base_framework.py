...
def _apply_runtime_model_path_overrides(model_config) -> None:
    # ActiveArena owns the release runtime. Keep the old StarVLA variable as a
    # fallback so existing checkpoint launchers continue to work.
    base_vlm = (
        os.environ.get("ACTIVEARENA_VLA_BASE_VLM")
        or os.environ.get("ACTIVEARENA_BASE_VLM")
        or os.environ.get("STARVLA_BASE_VLM")
    )
    if not base_vlm:
        return
...
def build_framework(cfg): # The single entry point for building different model frameworks
    ...
    # Apply release-time path overrides at the single construction boundary.
    # Training entry points call ``build_framework`` directly, while checkpoint
    # loading reaches it through ``from_pretrained``.  Keeping the override
    # here ensures both paths resolve the same local base VLM snapshot.
    _apply_runtime_model_path_overrides(cfg)
