...
class LeRobotSingleDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        history_rng = np.random.default_rng(safe_hash((self.epoch, index, "history")))
    ...
    def _history_sampling_strategy(self) -> str:
        return str(self._history_cfg_get("sampling_strategy", "fixed_stride"))
    ...
    def _resolve_current_video_keys(self, history_video_key: str) -> list[str]:
        configured_keys = self._history_cfg_get(
            "current_camera_keys",
            self._history_cfg_get("current_view_keys", None),
        )
        if configured_keys is None:
            return [history_video_key]
        if isinstance(configured_keys, str):
            configured_keys = [configured_keys]

        current_video_keys = [self._normalize_video_key(key) for key in configured_keys]
        if not current_video_keys:
            raise ValueError("history.current_camera_keys must contain at least one camera key")

        video_keys = self.modality_keys.get("video", [])
        missing_keys = [key for key in current_video_keys if key not in video_keys]
        if missing_keys:
            raise ValueError(
                f"Configured current camera keys {missing_keys} not found in video keys: {video_keys}"
            )
        return current_video_keys
    ...
    @staticmethod
    def _linspace_history_indices(base_index: int, count: int) -> list[int]:
        count = min(max(int(count), 0), max(int(base_index), 0))
        if count == 0:
            return []
        if count == 1:
            return [int(base_index) - 1]
        return np.rint(np.linspace(0, int(base_index) - 1, num=count)).astype(int).tolist()

    def _history_frame_indices(
        self,
        base_index: int,
        ...
        if mode == "action_keyframe":
            strategy = self._history_sampling_strategy()
            max_count = max(int(max_history or 0), 0)
            available_count = max(int(base_index), 0)

            if strategy == "fixed_count_uniform":
                history_indices = self._linspace_history_indices(base_index, max_count)
                return history_indices + [base_index]

            if strategy in {"random_count_uniform", "random_count_random"}:
                rng = rng if rng is not None else np.random.default_rng(int(base_index))
                count = int(rng.integers(0, min(max_count, available_count) + 1))
                if strategy == "random_count_uniform":
                    history_indices = self._linspace_history_indices(base_index, count)
                elif count == 0:
                    history_indices = []
                else:
                    history_indices = sorted(
                        int(index) for index in rng.choice(available_count, size=count, replace=False)
                    )
                return history_indices + [base_index]

            if strategy != "fixed_stride":
                raise ValueError(
                    f"Invalid history.sampling_strategy={strategy!r}; expected fixed_stride, "
                    "fixed_count_uniform, random_count_uniform, or random_count_random"
                )
            if max_history == 0:
                return [base_index]
    ...
    def _pack_history_images(
        self,
        trajectory_id: int,
        base_index: int,
        history_rng: np.random.Generator | None = None,
    ) -> tuple[list[Image.Image], list[int], list[int]]:
        history_video_key = self._resolve_history_video_key()
        current_video_keys = self._resolve_current_video_keys(history_video_key)
        sampled_indices = self._history_frame_indices(base_index, rng=history_rng)
        history_indices = sampled_indices[:-1]

        frames = []
        aligned_frame_indices = []
        if history_indices:
            history_frames = self.get_video(
                trajectory_id,
                history_video_key,
                base_index,
                step_indices=np.asarray(history_indices, dtype=int),
            )
            frames.extend(history_frames)
            aligned_frame_indices.extend(history_indices)

        for video_key in current_video_keys:
            current_frames = self.get_video(
                trajectory_id,
                video_key,
                base_index,
                step_indices=np.asarray([base_index], dtype=int),
            )
            if len(current_frames) != 1:
                raise ValueError(
                    f"Expected one current frame from {video_key}, got {len(current_frames)}"
                )
            frames.append(current_frames[0])
            aligned_frame_indices.append(base_index)

        resize_size = self._image_resize_size()
        images = [Image.fromarray(frame).resize(resize_size) for frame in frames]
        head_frame_indices = history_indices + [base_index]
        return images, aligned_frame_indices, head_frame_indices
    ...
    def _pack_sample(
        ...
        if self._history_enabled() and trajectory_id is not None and base_index is not None:
            step_images, image_frame_indices, history_indices = self._pack_history_images(
                trajectory_id,
                base_index,
                history_rng=history_rng,
            )
            num_history_frames = max(len(history_indices) - 1, 0)
        else:
            step_images = []
            resize_size = self._image_resize_size()
        ...
        if history_indices is not None:
            ...
            sample["image_frame_indices"] = image_frame_indices
            sample["state_frame_indices"] = history_indices
            sample["state_time_offsets"] = [int(index) - int(base_index) for index in history_indices]
            sample["history_mode"] = self._history_mode()
            sample["history_sampling_strategy"] = self._history_sampling_strategy()
            sample["num_frames"] = len(step_images)
            sample["num_history_frames"] = num_history_frames
            sample["num_current_frames"] = len(step_images) - num_history_frames
        else:
            sample["num_frames"] = len(step_images)
...
class LeRobotMixtureDataset(Dataset):
    ...
    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        ...
        strategy = self._history_sampling_strategy()
        reserve_full_history_window = self._requires_full_history() or strategy in {
            "random_count_uniform",
            "random_count_random",
        }
        if reserve_full_history_window and self._history_mode() == "action_keyframe":
            max_history = self._max_history_frames()
            if strategy in {
                "fixed_count_uniform",
                "random_count_uniform",
                ...
                min_base_index = int(max_history or 0)
            else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                while True:
                    ...
                    history_seed_parts = (self.mode, self.epoch, index, self.seed, "history")
                    if self.mode == "train":
                        history_seed_parts += (self._getitem_count,)
                    history_seed = safe_hash(history_seed_parts)
                    sample = dataset._pack_sample(
                        data,
                        trajectory_id=trajectory_id,
                        base_index=step,
                        history_rng=np.random.default_rng(history_seed),
                    )
